def siirqr(newTable,episodes_IN,beta1_IN,beta2_IN):
    import numpy as np
    ...
    class Q_Values():
        def __init__(self,Discretization_num_steps,num_actions,MAX,MIN):
            ...
            if loadit:
                ...
                print("table loaded")
                spying=self.q_table.any(axis=1)[0]
                print(f"ho trovato {len(spying)} valori non aggiornati")
    ...
    class Environment():
        ...
        def take_action(self,action,current_step,explored):
            if action==0:
            ...
            reward=self.compute_reward(current_step,explored)
        ...
        def compute_next_state(self):
            ...
            #print(f" infected are {self.current_infected} because beta is {self.beta0}")
    ...
    class Agent():
      ...
      def select_action(self,state,Q_Learn,beta,b_3):
        ...
        while not action_available(beta,b_3,action):
          action_vec[action]=np.min(action_vec)-1
    ...
    # def plotit(Env,t_stop,t_prev): ##$$$$ DA METTERE A POSTO
    ...
    #np.save('Q_Table_SIIR', Q_Learn.q_table)
    #print("saved")
...
#siirqr(newTable,20000,0.16,0.04)