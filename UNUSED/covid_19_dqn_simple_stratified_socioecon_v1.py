...
# -*- coding: utf-8 -*-
    ...
    images_dir = os.path.join(my_path,'static\\images\\Simple Stratified SocioEcon')
    ...
    class Environment(object):
        ...
        def instant_reward(self, delta_infect, step):
          ...
          #rew_socioecon=(self.beta-0.2)*weight_socioecon
          tot_r1=self.current_r1_u[-1]+self.current_r1_m[-1]+self.current_r1_o[-1]
          rew_socioecon=(self.beta-0.2)*math.exp((1-self.infected+tot_r1)*10)*weight_socioecon
          rew_time=-weight_time*step
          
          self.rew_cum=self.rew_cum+rew_inf+rew_socioecon+rew_time
          print(f"rew_inf is {rew_inf}, rew_socioecon is {rew_socioecon} at step {step}")
          #print(f"rew_inf is {rew_inf}, rew_socioecon is {rew_socioecon}, rew_time is {rew_time} at step {step}")
          return rew_inf+rew_socioecon+rew_time
        ...
        def step(self, action, step, show):
          ...
          if (step >= 100) or (self.infected >= 0.8) or (self.infected<=self.end_episode) : #end episode condition,
            ...
            if self.infected<=self.end_episode:
              reward = -self.terminate_reward(self.delta_infected, step)
            else:
    ...
    b_size = 1500 #batch size: dimension of each set of experiences sent to the network to be updated
    memory = np.zeros((memory_size, 37)) # 32 = State(16) + Action(1) + Next State(16) + Reward(1) + Beta(2) + Done(1)
    ...
    def run_deepQ(env, net, net2):
      ...
      for episode in range(EPISODES):
        ...
        if show:
          ...
          #fig=plt.figure()
          txt=f"Stratified With Reward function -self.infected/self.end_episode*1+(self.beta-0.2)*500-step*20 and 3 layer Networks got {reward} reward"
    ...
    state_dict=torch.load(path_socioecon) #load weight file from drive
    ...
    torch.save(net.state_dict(),path_socioecon) #save weights file in Drive
