def sirqr():
    ...
    global images_dir
    images_dir = os.path.join(my_path,'static\\images\\SIRQR')
    ...
    beta0=b1+b2   #probabilita di contagio da incontro a rischio
    ...
    class Q_Values():
        def __init__(self,Discretization_num_steps,num_actions,MAX,MIN):
            ...
            if loadit:
                ...
                #100x100x5 with initial random Q values between -2 and 0
                self.q_table=np.random.uniform(low=-0.1,high=0, size=(self.DISCRETE_OS_SIZE + [num_action]))
                print("Table: Randomly Generated")
    ...
    class Environment():
        ...
        def reset(self):
            ...
            self.history=np.array([self.current_susceptible,self.current_infected])
    ...
    def plotit0(s_path,i_path,List,Env):
    ...
    def plotit1(List,Env):
        #t_vec=np.linspace(0,len(s_path)-1,len(s_path)) DAAAAAAMNNNN c'era questooooooooooooo!!!
        s_path = Env.current_susceptible
        i_path = Env.current_infected
        r_path= np.ones((len(s_path),1))-s_path-i_path
        plt.plot(t_vec[0:10],s_path,t_vec[0:10],i_path)
        plt.title("S-I")
        plt.plot(t_vec[0:10],s_path,t_vec[0:10],i_path,t_vec[0:10],r_path)
        plt.show()
        ...
        axs[0,0].plot(t_vec[0:10],s_path,label='susceptibles')
        axs[0,0].set_title('susceptibles')
    
        axs[0,1].plot(t_vec[0:10],i_path,label = 'infected')
        axs[0,1].set_title('infected')
        ...
        axs[1,0].plot(t_vec[0:10],r_path[-1,:])
        axs[1,0].set_title('recovered')
    ...
    def plotit2(Env):
        #t_vec=np.linspace(0,len(s_path)-1,len(s_path)) DAAAAAAMNNNN c'era questooooooooooooo!!!
        s_pathpl2 = Env.susceptible
        i_pathpl2 = Env.infected
        r_pathpl2 = np.ones((len(s_pathpl2),1))-s_pathpl2-i_pathpl2
        t_vecpl2 = range(len(s_pathpl2))
        
        fig, axs = plt.subplots(2,1)
    
        axs[0].plot(t_vecpl2,s_pathpl2,label='susceptibles')
        axs[0].set_title('susceptibles')
    
        axs[1].plot(t_vecpl2,i_pathpl2,label = 'infected')
        axs[1].set_title('infected')
        
        plt.show()
        
        cnt=Env.count_action
        cnt_niente=cnt[4]-cnt[3]-cnt[2]-cnt[1]-cnt[0]
        cnt2 = [cnt[0],cnt[1],cnt[2],cnt[3],cnt_niente]
        
        fig, axs = plt.subplots(2,2)
        axs[0,0].bar(range(5), cnt2, label = ['chiudo scuole','apro scuole','chiudo mezzi','apro mezzi','niente'])
        axs[0,0].set_title('actions')
    
        axs[1,0].plot(t_vecpl2,Env.history[0,:])
        axs[1,0].set_title('susceptibles')
    
        axs[0,1].plot(t_vecpl2,Env.history[1,:])
        axs[0,1].set_title('infected')
    
        axs[1,1].plot(t_vecpl2,r_pathpl2[-1,:])
    ...
    Env=Environment(beta_1,beta_2,q,x_0)
